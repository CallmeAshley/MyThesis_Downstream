...
def parse_args():
    ...
    parser.add_argument('--resume', type=str, default='/mai_nas/LSH/MRSpineSeg_Challenge_SMU/unet/mae_1300_seed6343_freeze/model/Best_DSC.pth', help='load pretrained model path')
    parser.add_argument('--pretrain', type=str, default=None, help='load pretrained model path')
     
    parser.add_argument('--output_dir', type=str, default='./mae_1300_seed6343_freeze/',help='Directory name to save the model, log, config')
    ...
    parser.add_argument('--gpus',default='7' ,type=str, help='gpu num')
...
with torch.no_grad():
    ...
    for t, batch in enumerate(dataloader_test):
        ...
        resized_output = resize_output_to_cropped_size(test_output, cropped_shape, is_label=False)
        ...
        print("[Test set name: %s, DSC: %.4f, , HD: %.4f, SD: %.4f, Precision: %.4f, Recall: %.4f, Specificity: %.5f]"%(test_name[0], dsc_test.item(), 
        ...
        plt.subplot(1,3,2)
        plt.title('output')
        plt.imshow(test_output[0,1,:].cpu().detach().numpy(),cmap='gray')
        plt.subplot(1,3,3)
        plt.title('label')
