...
def parse_args():
    ...
    parser.add_argument('--pretrain2', type=str, default=None, help='load pretrained model path')
     
    parser.add_argument('--output_dir', type=str, default='./try/',help='Directory name to save the model, log, config')
    ...
    parser.add_argument('--freeze', type=str, default='false', help='true or false')
...
#---------------------------------------------------------------------------------------------#
if (args.resume is not None) or (args.pretrain2):
    if args.resume is not None:
        state_dict = torch.load(os.path.join(args.resume))
        net.load_state_dict(state_dict, strict=False)
    else:
        state_dict = torch.load(args.pretrain2)
...
for epoch in range(args.epoch):
    ...
    for i, train_data in enumerate(dataloader_train):
        ...
        train_output2 = net(train_input)
        train_output2 = train_output2['out']
        
        
        
        #---------------------------Ensemble code is here---------------------------#
